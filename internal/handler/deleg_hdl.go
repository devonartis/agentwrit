...
func (h *DelegHdl) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, deleg.ErrDepthExceeded):
			WriteProblem(r.Context(), w, http.StatusForbidden, "scope_violation", "delegation depth limit exceeded", r.URL.Path)
		case errors.Is(err, deleg.ErrDelegateNotFound):
			WriteProblem(r.Context(), w, http.StatusNotFound, "not_found", "delegate agent not found", r.URL.Path)
		default:
			WriteProblem(r.Context(), w, http.StatusInternalServerError, "internal_error", "delegation failed", r.URL.Path)
		}
		return
	}
