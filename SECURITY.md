...
- **In-Memory State**: Token state is stored in memory. Broker restarts will clear state, including pending authorization requests.
...
- **No Persistence Layer**: There is no built-in persistent storage. Audit logs and tokens exist only in the running process.
