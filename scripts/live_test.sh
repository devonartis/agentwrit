...
go build -o "$PROJECT_ROOT/bin/broker" "$PROJECT_ROOT/cmd/broker"
...
AA_SEED_TOKENS=true \
AA_LOG_LEVEL=quiet \
  "$PROJECT_ROOT/bin/broker" > /tmp/broker_live_test.log 2>&1 &
BROKER_PID=$!
...
check_json "POST /v1/admin/auth (valid)" POST "$BASE/v1/admin/auth" 200 \
  '{"client_id":"admin","client_secret":"live-test-secret-32bytes-long!!"}'
