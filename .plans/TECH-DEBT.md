...
| TD-S08 | `docs/api.md` — references OIDC discovery, JWKS, sidecar endpoints that don't exist in core | High | `docs/api.md` | B0 removed sidecar section but OIDC/JWKS endpoint docs may still reference features not in core. Also still shows `client_id`/`client_secret` in examples. Rewrite after migration. |
| TD-S09 | `docs/architecture.md` — references sidecar subsystem, OIDC federation, old auth flow | High | `docs/architecture.md` | Architecture doc describes the full agentauth system including sidecar + OIDC. Core only has broker. Rewrite after B6. |
| TD-S10 | `.claude/skills/broker-up/SKILL.md` — old secret `live-test-secret-32bytes-long!!` in some sections | Medium | `.claude/skills/broker-up/SKILL.md` | Cowork updated some sections but VPS mode and smoke test sections may still have the old `!!` secret. Should use `live-test-secret-32bytes-long-ok` or `aactl init` flow. |
| TD-S11 | `docker-compose.mtls.yml` / `docker-compose.tls.yml` — may reference sidecar service | Medium | `docker-compose.mtls.yml`, `docker-compose.tls.yml` | B0 removed `docker-compose.uds.yml` but mTLS and TLS compose files may still have sidecar service definitions. Verify and clean. |
